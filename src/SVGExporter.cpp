...
void SVGExporter::exportSVG(const std::string& filePath){
	...
	if(svgFile.is_open()){
		...
		// Calculate SVG frame size by finding min/max element locations.
		float width = -1;
		float height = -1;
		...
		// Output building polygons.
		svgFile << "<g id=\"map\">\n<g id=\"Polygons\" style=\"stroke-linejoin:round\">\n";
		svgFile << "<g id=\"buildings\" style=\"fill:#BCA9A9\">\n";

		for(size_t i = 0; i < buildings.size(); ++i){
			if(!buildings[i].exportStatus()) // Skip buildings marked for non-export.
				continue; 
		...
		for(size_t i = 0; i < paths.size(); ++i){
			if(!paths[i].exportStatus()) // Skip paths marked for non-export.
				continue;

			const std::vector<float>& pathPoints = paths[i].getPathPoints();

			// Road style information.
		...
		if(exportGrid){
			std::stringstream ss;
			ss << grid.lineSize;
			svgFile << "<g id=\"grid\" style=\"stroke:#000000;stroke-width:" << ss.str() << "\">\n";;
